...
class EA_AdminController extends EA_Controller
{
    ...
    public function getAdmin(): string
    {
        ...
        // Datenbank-Integrität nur auf dem Dashboard und der DB-Seite prüfen, um Last zu sparen
        if ($_GET['doc'] === "dashboard" || $_GET['doc'] === "db" || $_GET['doc'] === null) {
    ...
    private function getServerNetworkIP(): array
    {
        ...
        // Add the host IP if it's not localhost, in case the user accesses via their LAN IP
        $httpHost = $_SERVER['HTTP_HOST'] ?? '';
        if ($httpHost && strpos($httpHost, 'localhost') === false && strpos($httpHost, '127.0.0.1') === false) {
            // HTTP_HOST usually includes the port, so we append the project folder
            $meldungen[] = (isset($_SERVER['HTTPS']) ? 'https://' : 'http://') . $httpHost . '/' . $projektOrdner . '/';
        }
